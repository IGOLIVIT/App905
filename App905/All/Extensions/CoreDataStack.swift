...
class CoreDataStack {
    ...
    func deleteIdea(withIdTitle name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<IdeasModel> = IdeasModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "idTitle == %@", name)
    ...
    func deleteWork(withWoTitle name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<WorkModel> = WorkModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "woTitle == %@", name)
    ...
    func deleteProj(withPrTitle name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<ProjModel> = ProjModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "prTitle == %@", name)
